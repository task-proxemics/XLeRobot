...
可以在其他文件夹中调用telegrip的VR功能并读取打印VR控制信息
"""
...
# 设置telegrip文件夹的绝对路径
TELEGRIP_PATH = "/home/vec/lerobot/new/telegrip"

def setup_telegrip_environment():
    """设置telegrip环境"""
    # 添加telegrip路径到Python路径
    ...
    # 设置环境变量
    os.environ['PYTHONPATH'] = f"{TELEGRIP_PATH}:{os.environ.get('PYTHONPATH', '')}"
...
class VRMonitor:
    ...
    def initialize(self):
        ...
        # 导入模块
        TelegripConfig, VRWebSocketServer, ControlGoal, ControlMode = import_telegrip_modules()
        ...
        print("✅ VR Monitor initialized successfully")
...
def main():
    """主函数"""
    print("🎮 VR Monitor - Telegrip VR Control Information Monitor")
    print("=" * 60)
