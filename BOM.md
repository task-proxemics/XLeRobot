...
> [!IMPORTANT]
> **XLeRobot** is designed for **maximum power**, which is why it only comes in **a 12V version**. If you're using pre-assembled SO100 and Lekiwi components, it's suggested use their 12V motor versions. While 7.4V motors might function, we don't recommend them since they're untested and won't deliver optimal performance.

> [!TIP]  
> While every robot enthusiast should try hands-on assembly at least once, you can purchase [pre-assembled kits](https://github.com/TheRobotStudio/SO-ARM100#kits) for SO100/SO101 arms at a higher price. The SO100 arms require the most assembly time—about 4-6 hours for two arms. Once the arms are complete, the remaining assembly takes just 1-2 hours.
...
If you already have two SO100 Arms and one Lekiwi base, you'll need only these additional parts

(please also review the Build From Scratch section below, as the parts list differs slightly)
...
- You can use a similar cart not from IKEA; however, this may require minor adjustments to the [3D printed part](3D_Models/3D_models_for_printing/XLeRobot_special/base_connector.stl).
- While this battery 🔋 is expensive 💵, its performance and reliability significantly outperform standard power banks. (Similar to electric vehicles 🚗, the battery is one of the costliest components.)
- You can use **other batteries**, but ensure the maximum output capacity can adequately power 2 arms, 3 wheels, and a RaspberryPi (~160W).
- Two Type-C to DC cables (12V) connect the motor control board to the power supply.


## 🌿 Build From Scratch 🌿

For those starting from scratch (without SO100 arms or Lekiwi), you'll need to build the arms and Lekiwi base first, in addition to gathering the parts from *Extra Parts for XLeRobot*.

> [!NOTE]
> The parts list for 2× SO100 arms and 1 Lekiwi base in this repo includes fewer components than the original bills (primarily cable and battery-related items). If you're building **XLeRobot** from scratch, you can use this repo's bill directly. However, we recommend checking the other two repos ([SO100](https://github.com/TheRobotStudio/SO-ARM100/tree/main), [Lekiwi](https://github.com/SIGRobotics-UIUC/LeKiwi/blob/main/BOM.md)) as well for potentially better-priced options.

> [!NOTE]
> The new [SO101 arm](https://github.com/TheRobotStudio/SO-ARM100) was released (2025.4.28) with improved wiring and easier assembly. This update does not affect the parts list below if you are only making follower arms.

### 🦾 2x SO100 Arms 🦾
| Part | Amount | Unit Cost (US) | Buy US | Unit Cost (EU) | Buy EU | Unit Cost (RMB) | Buy CN |
|---|---|---|---|---|---|---|---|
| STS3215 Servo (12V) | 12 | $14 | [Wowrobo](https://shop.wowrobo.com/products/feetech-sts3215-servo-12v-30kg-high-torque-servo-for-so-arm100) | €13 | [Alibaba](https://www.alibaba.com/product-detail/6PCS-7-4V-STS3215-Servos-for_1600523509006.html) | ￥108 | [TaoBao](https://item.taobao.com/item.htm?id=712179366565&skuId=5268252241438) |
...
> [!NOTE] 
> Both SO100 Arms are configured as follower arms in the hardware setup. To save money while building only two arms, you can use one of them as the leader arm for testing teleop—there's no need to remove the motor gears.
...
- You can use any brand of cables (USB-C cable must be fast-charging type), microSD card, and Raspberry Pi 5
...
These components will allow you to replicate Demo 0.1.0, enabling leader-follower arm control and direct PC control of both arms.


## 🧩 Optional Add-on Features 🧩

> [!NOTE]
> Today (2025.4.28) the [SO101 arm](https://github.com/TheRobotStudio/SO-ARM100) was released with improved wiring and easier assembly, featuring some changes to the leader arms.
...
- For more add-ons and accessories, check out the [SO100 arm repository](https://github.com/TheRobotStudio/SO-ARM100/tree/main?tab=readme-ov-file#optional-hardware) and Discord group discussions
