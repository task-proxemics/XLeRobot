...
> [!NOTE] 
> **XLeRobot 0.1.0** 正式发布！当前版本包括详细的**材料清单**、**3D打印模型和说明**，以及**逐步组装指南**。虽然代码尚未提供，但您可以使用原始Lekiwi代码库在单臂版本上**运行远程操作测试**（由另一个跟随臂控制）。这里是[我本月正在为XLeRobot 0.2.0开发的一些升级](Updates.md)，但我只会在所有计划和新演示完全完成后才会全面更新整个仓库。
...
> [!NOTE] 
> 如果你完全不懂编程，请至少花一天时间熟悉基本的Python、Ubuntu和Github(借助Google和AI的帮助)。至少你应该知道如何设置ubuntu系统、git克隆、pip安装、使用解释器(VS Code、Cursor、Pycharm等)以及直接在终端中运行命令。
...
> [!NOTE]
> 如果您基于此仓库构建XLeRobot，您将对其对您或他人造成的所有身体和精神损害负全部责任。
