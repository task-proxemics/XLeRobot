...
> [!NOTE] 
> **XLeRobot 0.1.0** is officially out! The current version includes a detailed **bill of materials**, **3D printing models and instructions**, and a **step-by-step assembly guide**. While the code is not yet available, you can **run the teleoperation test** on a single arm version (controlled with the other follower arm) directly using the original Lekiwi codebase. Here are some [ongoing upgrades I am currently working on this month for XLeRobot 0.2.0](Updates.md), but I will only fully update the whole repo after all the plans and new demos are fully finished.
